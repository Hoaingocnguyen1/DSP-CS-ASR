...
class DSP_Whisper(nn.Module):
    ...
    def __init__(
        ...
        # 7. Dynamic Acoustic Gate Injection (V2 Architecture)
        ...
        self.gate_net = nn.Sequential(
            nn.Linear(num_languages, hidden_size),
            nn.GELU(),
    ...
    def forward(self, wav, decoder_input_ids, wav_lens=None):
        ...
        # 3. Dynamic Tanh Gate Injection (V2 method)
        # Calculate dynamic per-frame gate based on LID probabilities
        prompt_projected = self.prompt_dropout(self.prompt_proj(prompt))
        lid_probs = torch.softmax(lid_logits, dim=-1) # [B, 1500, num_languages]
        dynamic_gate = self.gate_net(lid_probs)       # [B, 1500, input_size]
        
        injection = torch.tanh(dynamic_gate) * prompt_projected
    ...
    def get_encoder_out(self, wav):
        ...
        dynamic_gate = self.gate_net(lid_probs)
        injection = torch.tanh(dynamic_gate) * prompt_projected
        encoder_out = encoder_out + injection
